...
@property (nonatomic, assign) NSInteger oneComSelctRow;
@property (nonatomic, assign) NSInteger twoComSelctRow;
@end
...
{
    if (_toolBar==nil) {
        _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, kScreenH-self.bounds.size.height-44, kScreenW, 44)];
...
- (void)finishClick:(UIButton *)btn
{
    if (self.didSelectBlock) {
        ...
        [dic setObject:[NSNumber numberWithInteger:self.oneComSelctRow] forKey:@"rowOne"];
        [dic setObject:[NSNumber numberWithInteger:self.twoComSelctRow] forKey:@"rowTwo"];
        self.didSelectBlock(dic);
    }
    [self close];
}
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if(component==0){
       return self.datas[component].count;
        ...
        NSArray *aray=self.datas[component];
        if(self.oneComSelctRow>=aray.count){
            return 0;
        }
        NSArray *childArray=aray[self.oneComSelctRow];
...
{
    if(component==0){
        self.oneComSelctRow=row;
    }
    if(component==1){
        self.twoComSelctRow=row;
    }
