...
- (void)addNoticeInfoLable:(CGRect)rect{
    ...
    [_noticeInfoLable setText:@"将二维码放入框内,即可自动扫描"];
...
- (void)initDataWithView:(UIView *)parentView{
        ...
        authStatus ==AVAuthorizationStatusDenied)  //用户已经明确否认了这一照片数据的应用程序访问
    {
        ...
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:nil message:@"请在IPhone的\"设置-隐私-相机\"选项中,允许biger访问你的相机" delegate:self cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    ...
    CGRect cropRect = CGRectMake(size.width / 2 - transparentArea.width / 2,
                                 size.height / 2 - transparentArea.height / 2,
                                 transparentArea.width,transparentArea.height);
