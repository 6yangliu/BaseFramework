...
@interface HKADScrollView ()<UIScrollViewDelegate>{
...
- (void)layoutUI {
    ...
    _bannerView.layer.cornerRadius=self.layer.cornerRadius;
    _bannerView.clipsToBounds=YES;
    _bannerView.delegate = self;
    _bannerView.bounces = YES;
    [self addSubview:_bannerView];
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0,
                                                           CGRectGetHeight(self.frame) - kPageViewHeight-self.width*0.11,
                                                           CGRectGetWidth(self.frame),
                                                           kPageViewHeight)];
    _bottomView.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.5];
    [self addSubview:_bottomView];
    _pageCtl = [[HKADPageControl alloc]initWithFrame:_bottomView.bounds];
...
- (void)makeBannerLayout:(NSArray *)images networkImageNumber:(NSUInteger)number {
    ...
    for (NSInteger i = 0; i < _imageCount; i++) {
        ...
        if (_networkImageCount > 0) {
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(HKADScrollView:networkLoadingWithImageView:imageUrl:index:)]) {
                [_delegate HKADScrollView:self networkLoadingWithImageView:imageView imageUrl:_imageUrls[index] index:index];
            }
    ...
    switch (_pageViewPosition) {
        case Left: {
            _pageCtl.frame=CGRectMake(0, 0, pageViewWidth, CGRectGetHeight(_bottomView.frame));
            if (_imageTitleArray) {
            ...
            break;
        case Centre: {
            _pageCtl.frame=CGRectMake((CGRectGetWidth(_bottomView.frame) - pageViewWidth) / 2, 0, pageViewWidth, CGRectGetHeight(_bottomView.frame));
        }
            break;
        case Right: {
            _pageCtl.frame=CGRectMake(CGRectGetWidth(_bottomView.frame) - pageViewWidth, 0, pageViewWidth, CGRectGetHeight(_bottomView.frame));
            if (_imageTitleArray) {
...
- (void)handleTapImageView:(UITapGestureRecognizer *)tapGesture {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(HKADScrollView:clickImageView:index:)]) {
        [_delegate HKADScrollView:self clickImageView:imageView index:index];
    }
