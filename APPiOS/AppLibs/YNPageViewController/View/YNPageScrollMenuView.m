...
- (void)setupOtherViews {
    ...
    if (self.configration.showScrollLine) {
        ...
        if(self.configration.lineWidth){
            self.lineView.yn_width = [self.configration.lineWidth floatValue];
        }else{
            self.lineView.yn_width = lineW + self.configration.lineLeftAndRightAddWidth * 2 - 2 * self.configration.lineLeftAndRightMargin;
        }
...
- (void)setDefaultTheme {
    ...
    if (self.configration.showScrollLine) {
        ...
        if(self.configration.lineWidth){
            self.lineView.yn_width = [self.configration.lineWidth floatValue];
            self.lineView.yn_centerX = currentButton.yn_centerX;
        }else{
...
- (void)adjustItemWithProgress:(CGFloat)progress
    ...
    if (self.configration.showScrollLine) {
        ...
        if (!self.configration.scrollMenu &&
            ...
            if(self.configration.lineWidth){
                self.lineView.yn_width = [self.configration.lineWidth floatValue];
                self.lineView.yn_centerX=lastButton.yn_centerX;
            }else{
