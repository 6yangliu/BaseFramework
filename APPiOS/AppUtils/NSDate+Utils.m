...
+ (NSString *)stringYearMonthDayWithDate02:(NSDate *)date
{
    if (date == nil) {
        date = NULL;
    }
...
+ (NSString *)stringYearMonthDayWithDate:(NSDate *)date format:(NSString *)f;
{
    if (date == nil) {
        date = NULL;
    }
...
+ (NSString *)timeFromTimeStamp:(NSString *)interval format:(NSString *)format{
   
    NSString *timeStampString  = interval;
    // iOS 生成的时间戳是10位
    NSTimeInterval intervalValue    =[timeStampString doubleValue] / 1000.0;
    NSDate *date               = [NSDate dateWithTimeIntervalSince1970:intervalValue];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
