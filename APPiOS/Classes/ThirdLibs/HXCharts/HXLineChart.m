...
- (void)setTitleArray:(NSArray *)titleArray{
    ...
    for (int i = 0; i < titleArray.count; i++) {
//        if ((_x + _margin + (_lineWidth - _margin * 2) / (titleArray.count - 1) * i - _titleWidth / 2) > 0) {
//
//        }
        ...
        titleLabel.textColor = [UIColor colorWithRed:145.0/255.0 green:157.0/255.0 blue:169.0/255.0 alpha:1.0];
...
//    [self.layer addSublayer:lineLayer];
    ...
    gradientLayer.frame = CGRectMake(0, 0, self.width, FitCount(200));
    gradientLayer.colors = @[(__bridge id)RGB(154, 203, 255).CGColor,(__bridge id)WHITE_COLOR.CGColor];
...
//    gradientLayer.locations=@[@0.0,@1.0];
    gradientLayer.locations = @[@(0.2f) ,@(0.5),@(1.0f)];
    ...
    [lineChartLayer addSublayer:gradientLayer];
