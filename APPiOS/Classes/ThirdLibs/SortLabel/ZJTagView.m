...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.tag = 0;
        if (self.inEditState) {
//            [cell.delButton addTarget:self action:@selector(delButtonUp:) forControlEvents:UIControlEventTouchUpInside];
//            cell.delButton.tag = indexPath.row;
        ...
        cell.tag = 1;
//        [cell.delButton addTarget:self action:@selector(delButtonDown:) forControlEvents:UIControlEventTouchUpInside];
//        cell.delButton.tag = indexPath.row;
    }
...
- (void)delButtonUp:(UIButton *)button{
    
    ZJTagViewCell *cell = (ZJTagViewCell *)button.superview;
    NSIndexPath *indexPath = [self.collectionView indexPathForCell:cell];
    NSInteger index = indexPath.row;
    
    if (_inEditState) {
        if (index == 0) return; // 不能编辑第一个
        // 添加新数据
        [self.unselectedItems addObject: self.selectedItems[index]];
        // 删除旧数据
        ...
        [_collectionView moveItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0] toIndexPath:newIndexPath];
        [_collectionView reloadData];
    }else{
        if (_delegate && [_delegate respondsToSelector:@selector(tagView:didSelectTagWhenNotInEditState:)]) {
            [_delegate tagView:self didSelectTagWhenNotInEditState:index];
        }
    }
}
...
- (UICollectionViewFlowLayout *)flowLayout {
    CGFloat width = ([UIScreen mainScreen].bounds.size.width - 70)/4.0;
    if (!_flowLayout) {
        UICollectionViewFlowLayout *flowLayout = [[UICollectionViewFlowLayout alloc] init];
        flowLayout.itemSize = CGSizeMake(width, 44);
