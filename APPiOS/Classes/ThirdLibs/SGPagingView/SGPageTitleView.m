...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure withShowSortButton:(BOOL)isShow{
    if (self = [super initWithFrame:frame]) {
        ...
        [self initialization];
        [self setupSubviewsWithShow:isShow];
    }
    return self;
}

+ (instancetype)pageTitleViewWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure withShowSortButton:(BOOL)isShow{
    return [[self alloc] initWithFrame:frame delegate:delegate titleNames:titleNames configure:configure withShowSortButton:isShow];
}
...
- (void)setupSubviewsWithShow:(BOOL)isShow{
    // 0、处理偏移量
    ...
    if (isShow) {
        UIButton *sortButton = [[UIButton alloc] initWithFrame:CGRectMake(SGPageTitleViewWidth - 30, 8, 16, 16)];
        [sortButton setImage:[UIImage imageNamed:@"rectangle8"] forState:UIControlStateNormal];
...
//        CGFloat bottomSeparatorW = 16.0;
        ...
        _bottomSeparator.backgroundColor = [UIColor colorWithRed:239.0/255.0 green:243.0/255.0 blue:245.0/255.0 alpha:1];
    }
